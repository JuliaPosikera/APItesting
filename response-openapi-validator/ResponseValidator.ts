...
import {
  UrlIsNotDescribedInOpenApiError,
  JSONSchemaMissingError,
  MultipleJSONSchemasDefinedError,
  JSONSchemaCannotBeCompiledError,
  ResponseDoesNotMatchJSONSchemaError,
} from "./Errors";
...
export class ResponseValidator {
  ...
  protected async loadApiDocs(): Promise<OpenAPI.Document> {
    if (this.cachedApi === null) {
      this.cachedApi = await SwaggerParser.dereference(
        this.options.openApiSpecPath
      );
    }
    return this.cachedApi;
  }

  protected async findMatchingPathInDocs(
    url: string
  ): Promise<OpenAPIV2.PathsObject & OpenAPIV3.PathsObject> {
    ...
    const matchingPaths = Object.keys(api.paths)
      .filter((template) => {
        try {
    ...
    return Object.fromEntries(
      matchingPaths.map((pth) => [pth, api.paths[pth]])
    );
  }
  ...
  public async assertResponse(response: ResponseToValidate): Promise<void> {
    if (!response) {
      throw new Error(
        "response argument is not defined. This is testing framework issue, not real bug"
      );
    }
    const findSchemaInPath: any = function (path: any) {
      if (path === undefined) {
        return undefined;
      }
      //Early return
      ...
      for (property in path) {
        if (
          path.hasOwnProperty(property) &&
          typeof path[property] === "object"
        ) {
    ...
    const matchingPaths = await this.findMatchingPathInDocs(
      response.requestUrl
    );
    const schemas = Object.values<
      OpenAPIV2.PathsObject & OpenAPIV3.PathsObject
    >(matchingPaths)
      .map((pathObj) =>
        findSchemaInPath(
          pathObj[response.method.toLowerCase()]?.responses[response.statusCode]
        )
      )
    ...
    } catch (jsonSchemaCompilationError: any) {
      throw new JSONSchemaCannotBeCompiledError(
        response,
